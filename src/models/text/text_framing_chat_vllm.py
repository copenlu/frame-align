...
def get_messages(model_code:str, article:str, task:str, task_prompt:str) -> list:
    ...
    if task == "Topic":
        ...
        messages = [{"role": "user", "content": task_prompt + POST_PROMPT}]
    return messages
...
def annotate_frames(model_code, data_file)-> None:
    ...
    for i, row in news_df.iterrows():
        ...
        for task, task_prompt in text_prompt_dict.items():
            ...
            messages.extend(get_messages(model_code, article_text, task, task_prompt))
        ...
        with open(f"./data/annotated/text/{data_name}_chat_{model_name_short}.jsonl", "a") as f:
