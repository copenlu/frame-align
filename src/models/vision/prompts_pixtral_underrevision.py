...
FRAMES_basic = f"""
    Economic - costs, benefits, or other finance related.
    Capacity and resources - availability of physical, human, or financial resources, and capacity of current systems.
    Morality - religious or ethical implications.
    Fairness and equality - balance or distribution of rights, responsibilities, and resources.
    Legality, constitutionality and jurisprudence - legal rights, freedoms, and authority of individuals, corporations, and government.
    Policy prescription and evaluation - discussion of specific policies aimed at addressing problems. 
    Crime and punishment - effectiveness and implications of laws and their enforcement. 
    Security and defense - threats to the individual, community, or nation. 
    ...
    None - no frame could be identified because of lack of information in the image. Example, a handshake with no other information, a logo of a company with no other information, a landscape with no other information, a person in a photo album with no other information, a person speaking with no other information about the content of the speech or person's identity, a formal event with  no other information, a person in formal attire with no other information, a news logo with no news, a sports event with no additional information, simple objects like vehicle/car/pen/paper/sign-boards/objects etc with no other information etc.
    """
...
And now for the image you see, look at frames list provided first and then choose the correct frame from the list of provided frames.
Identify the frame name associated with the larger theme that can evoke a latent message for readers of the article where this image is present.
Using this information, identify the frame present in the image.
Use only what is literally seen in the image to classify it.
If there is text in image, extract it and use the information fo framing if it contributes to a frame.
Your output should be in a json format with the fields "frame-justification" and "frame-name" .
Only identify the frame that is most relevant to the image.
The frame has to be one provided in the list.
If there is no frame from the list that is clearly communicated, choose "None".
"""
...
<image>
Your task is to carefully analyse the image and choose the correct frame from the above list.
Output your answer in a json format with the format:
{"frame-name": "<One of the frames from the list provided above>"}
...
# PROMPT_DICT_PIXTRAL = {
#     'normal': SYS_PROMPT + FRAMING_PROMPT + FRAMES + PROMPT_TASK + POST_PROMPT_FIX,
#     "normal_with_sptok" : prefix_instruction + SYS_PROMPT + FRAMING_PROMPT + FRAMES + PROMPT_TASK_ASSIS + POST_PROMPT_FIX,
#     "normal_no_fix": SYS_PROMPT + FRAMING_PROMPT + FRAMES + PROMPT_TASK,
#     'mini': SYS_PROMPT + FRAMING_PROMPT + FRAMES + MINI_PROMPT_TASK,
#     'normal_basic': SYS_PROMPT + FRAMING_PROMPT + FRAMES_basic + PROMPT_TASK+ POST_PROMPT_FIX,
#     'mini_basic': SYS_PROMPT + FRAMING_PROMPT + FRAMES_basic + MINI_PROMPT_TASK,
#     'normal_no_num': SYS_PROMPT + FRAMING_PROMPT + FRAMES_no_num + PROMPT_TASK + POST_PROMPT_FIX,
#     'mini_no_num': SYS_PROMPT + FRAMING_PROMPT + FRAMES_no_num + MINI_PROMPT_TASK
# }
PROMPT_DICT_PIXTRAL = {
